...
if ( ! empty( $_POST ) ) {
  ...
  if ( isset( $_POST['usuar'] ) && isset( $_POST['contras'] ) ) {
    ...
    // el signo ? significa que usaremos parametros para evitar la inyeccion SQL
        $sentencia = $con->prepare("SELECT * FROM clientes WHERE nombreC = ?");
    ...
    // y su valor del campo (a traves de su nombre) de la base de datos 
      if ( password_verify( $_POST['contras'], $usuario->contra ) ) { // contra = nombre campo tabla
        $_SESSION['identificador'] = $usuario->idcliente; // ID = nombre campo tabla
...
// si la variable de sesion ha sido definida
if (isset($_SESSION["identificador"])){
  echo '<script type="text/javascript">alert("BIENVENIDO");window.location.href="usuarios/index.php";</script>';
}else{
  session_destroy();
}
?>
